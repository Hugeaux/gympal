...
FriendlyEats.prototype.viewList = function(filters, filter_description) {
  ...
  var renderer = {
    ...
    display: function(doc) {
      var data = doc.data();
      data['.id'] = doc.id;
      data['go_to_gym'] = function() {
        that.router.navigate('/gyms/' + doc.id);
      };
...
FriendlyEats.prototype.initReviewDialog = function() {
  ...
  this.dialogs.add_review.listen('MDCDialog:accept', function() {
    ...
    var e = document.getElementById("vibe");
    var selectedValue = e.options[e.selectedIndex].value;

    var f = document.getElementById("money");
    ...
    that.addRating(id, {
      ...
      daypass: dialog.querySelector('input[name="daypass"]:checked').value,
      transport: dialog.querySelector('input[name="transport"]:checked').value,
...
FriendlyEats.prototype.viewGym = function(id) {
  var sectionHeaderEl;

  var that = this;
  return this.getGym(id)
    .then(function(doc) {
      var data = doc.data();
